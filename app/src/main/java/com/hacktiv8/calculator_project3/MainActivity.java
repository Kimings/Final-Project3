package com.hacktiv8.calculator_project3;
...
public class MainActivity extends AppCompatActivity {
    ...
    private Button button0, button1, button2, button3, button4, button5, button6, button7,
            ...
            buttonPower, buttonEqual, buttonClear, buttonPercent;
    ...
    private void solve() {
        if (input.split("\\+").length == 2){
            ...
            try{
                double d = Double.parseDouble(numbers[0]) +
                        Double.parseDouble(numbers[1]);
                output = Double.toString(d);
                newOutput = cutDecimel(output);
        ...
        if (input.split("\\^").length == 2){
            String numbers[] = input.split("\\^");
            try{
                double d =Math.pow(Double.parseDouble(numbers[0]),
                        Double.parseDouble(numbers[1]));
                output = Double.toString(d);
                newOutput = cutDecimel(output);
                outputText.setText(newOutput);
    ...
    private String cutDecimel (String number){
